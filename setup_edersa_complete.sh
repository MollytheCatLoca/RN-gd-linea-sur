#!/bin/bash
# Script COMPLETO para configurar proyecto EDERSA
# Copiar este archivo a gd-edersa-calidad y ejecutar desde allí

echo "🚀 CONFIGURACIÓN COMPLETA PROYECTO EDERSA"
echo "========================================="

# Verificar que estamos en el directorio correcto
if [[ ! -f "Mediciones Originales EDERSA.xlsx" ]]; then
    echo "❌ ERROR: No se encuentra 'Mediciones Originales EDERSA.xlsx'"
    echo "Asegúrate de estar en el directorio gd-edersa-calidad"
    exit 1
fi

echo "✅ Archivo EDERSA encontrado"
echo ""

# ===== PARTE 1: LIMPIEZA =====
echo "🧹 FASE 1: Limpiando proyecto clonado..."
echo "----------------------------------------"
...
# Eliminar documentación específica
...
rm -f docs/CLAUDE_METHODOLOGY.md
rm -f docs/project/
...
# Limpiar src - mantener solo estructura base
echo "💻 Limpiando código fuente..."
rm -rf src/*

# Limpiar dashboard
echo "🖥️ Limpiando dashboard antiguo..."
rm -rf dashboard/pages/
rm -rf dashboard/components/
rm -rf dashboard/utils/
rm -f dashboard/*.py

# Eliminar archivos del root no necesarios
echo "🗑️ Limpiando archivos raíz..."
rm -f EDERSA_PROJECT_SETUP.md
rm -f cleanup_for_edersa*.sh
rm -f setup_edersa*.sh
rm -f models/.gitkeep

# ===== PARTE 2: CREAR ESTRUCTURA =====
echo ""
echo "🏗️ FASE 2: Creando estructura EDERSA..."
echo "----------------------------------------"

# Crear directorios
mkdir -p src/{inventory,quality,clustering,optimization}
...
touch docs/methodology/.gitkeep

# ===== PARTE 3: CREAR ARCHIVOS PYTHON =====
echo ""
echo "🐍 FASE 3: Creando módulos Python..."
echo "----------------------------------------"

# CREAR transformer_loader.py
echo "📝 Creando transformer_loader.py..."
cat > src/inventory/transformer_loader.py << 'PYTHON_EOF'
"""
...
# CREAR quality_analyzer.py
echo "📝 Creando quality_analyzer.py..."
cat > src/quality/quality_analyzer.py << 'PYTHON_EOF'
"""
...
# CREAR geographic_clustering.py
echo "📝 Creando geographic_clustering.py..."
cat > src/clustering/geographic_clustering.py << 'PYTHON_EOF'
"""
...
# CREAR app_edersa.py
echo "📝 Creando dashboard app_edersa.py..."
cat > dashboard/app_edersa.py << 'PYTHON_EOF'
"""
...
if __name__ == '__main__':
    app.run_server(debug=True, port=8051)  # Puerto 8051 para no conflicto
PYTHON_EOF

# ===== PARTE 4: CREAR ARCHIVOS DE CONFIGURACIÓN =====
echo ""
echo "📋 FASE 4: Creando archivos de configuración..."
echo "----------------------------------------"

# CREAR requirements_edersa.txt
echo "📝 Creando requirements_edersa.txt..."
...
# ===== PARTE 5: MOVER ARCHIVO EDERSA =====
echo ""
echo "📂 FASE 5: Organizando datos..."
echo "----------------------------------------"

# Mover archivo Excel a data/raw si existe
if [[ -f "Mediciones Originales EDERSA.xlsx" ]]; then
    mv "Mediciones Originales EDERSA.xlsx" data/raw/
    echo "✅ Archivo EDERSA movido a data/raw/"
fi
...
echo "✓ Estructura EDERSA creada"
echo "✓ Módulos Python instalados"
...
echo ""
echo "📊 El dashboard estará disponible en: http://localhost:8051"
echo ""
echo "💡 Recuerda hacer el commit inicial:"
echo "git add ."
echo "git commit -m 'Initial setup: EDERSA Quality Analysis Project'"
echo ""
echo "¡Proyecto EDERSA listo para usar! 🎉"