# SETUP PROYECTO EDERSA - ANÁLISIS DE CALIDAD Y GD
## Guía completa para crear el nuevo proyecto basado en el framework CLAUDE
...
cd ~/Proyects  # o tu directorio de proyectos
git clone https://github.com/MollytheCatLoca/RN-gd-linea-sur.git gd-edersa-calidad
cd gd-edersa-calidad

# 2. Cambiar el remote
git remote remove origin
git remote add origin https://github.com/[tu-usuario]/gd-edersa-calidad.git

# 3. Ejecutar script de limpieza (ver sección abajo)
bash cleanup_for_edersa.sh

# 4. Copiar datos EDERSA
cp ~/Proyects/estudio-gd-linea-sur/Mediciones\ Originales\ EDERSA.xlsx data/raw/
...
# 6. Commit inicial
git add .
git commit -m "Initial commit: EDERSA Quality Analysis Project"
git push -u origin main
```

---

## 📁 ARCHIVOS A ELIMINAR DEL PROYECTO CLONADO

Crear archivo `cleanup_for_edersa.sh`:

```bash
#!/bin/bash
# Script de limpieza para adaptar a EDERSA

echo "🧹 Limpiando proyecto para EDERSA..."

# Eliminar datos específicos de Línea Sur
...
# Eliminar scripts específicos de Línea Sur
rm -rf scripts/analysis/analisis_*menucos*
rm -rf scripts/process_*
rm -f scripts/test_dc_*

# Limpiar tests específicos
rm -rf tests/results/
rm -f tests/test_phase5_*
...
## 📊 ANÁLISIS DE DATOS EDERSA (YA REALIZADO)

### Resumen de hallazgos:
- **14,025 transformadores** en toda la provincia
- **8,022 con mediciones de calidad** (57%)
- **2,731 penalizados o fallidos** (34% de los medidos)
- **14 sucursales** principales
- **133 alimentadores** únicos
- **Coordenadas geográficas** disponibles para 14,012 transformadores

### Estructura de datos:
```python
# Columnas principales del Excel EDERSA
columnas_clave = {
    'Codigoct': 'ID único del transformador',
    'N_Sucursal': 'Sucursal/Ciudad',
    'Alimentador': 'Nombre del alimentador',
    'Potencia': 'Potencia en kVA',
    'Q_Usuarios': 'Cantidad de usuarios',
    'Resultado': 'Estado de calidad (Correcta/Penalizada/Fallida)',
    'Coord_X': 'Longitud',
    'Coord_Y': 'Latitud',
...
## 🔧 NUEVOS MÓDULOS PARA EDERSA

### 1. `src/inventory/transformer_loader.py`
...
### 2. `src/quality/quality_analyzer.py`
...
### 3. `src/clustering/geographic_clustering.py`
...
1. **Crear directorio del proyecto** fuera del actual
2. **Copiar este archivo** al nuevo directorio
3. **Ejecutar los comandos** de la sección "Inicio Rápido"
4. **Copiar el Excel EDERSA** a `data/raw/`
5. **Ejecutar el dashboard** para ver primeros resultados

## 🎉 RESULTADO ESPERADO

Al completar estos pasos tendrás:
- ✅ Proyecto independiente para EDERSA
- ✅ Dashboard funcional con análisis básico
- ✅ Identificación de zonas críticas
- ✅ Propuesta de ubicaciones para GD
- ✅ Base para análisis más profundos

---

**Nota**: Este proyecto reutiliza la arquitectura CLAUDE pero está específicamente adaptado para el análisis de inventario de transformadores, no series temporales.